...
public class StorageServiceImpl implements StorageService {
     ...
     */
    public StorageServiceImpl(String jdbcUrl, String user, String password, int poolSize) throws SQLException {
        ...
        config.setPoolName("securenet-storage");
        this.dataSource = new HikariDataSource(config);
        initializeSchema();
        System.out.println("[StorageServiceImpl] Connected to PostgreSQL pool: " + jdbcUrl);
    }
    ...
    public void saveVideoClip(VideoClip clip, byte[] rawBytes) throws IllegalArgumentException {
        ...
        try (Connection conn = dataSource.getConnection()) {
            ...
            try {
                ...
                try (PreparedStatement ps = conn.prepareStatement(clipSql)) {
                ...
                try (PreparedStatement ps = conn.prepareStatement(bytesSql)) {
            ...
            throw new RuntimeException("Failed to acquire connection for saveVideoClip", e);
        }
    }
    ...
    public void saveFirmwareBinary(String deviceTypeKey, String firmwareVersion, byte[] binaryBytes) {
        try (Connection conn = dataSource.getConnection()) {
            ...
            try {
                ...
                try (PreparedStatement ps = conn.prepareStatement(fwSql)) {
                    ps.setString(1, deviceTypeKey);
                    ps.setString(2, firmwareVersion);
            ...
            throw new RuntimeException("Failed to acquire connection for saveFirmwareBinary", e);
        }
    }
    ...
    /**
     * Atomically computes max(current, candidate) + 1 and persists it.
     *
     * <p>Implements DS Problem #7 (Lamport Clocks) correctly under concurrency.
     * The entire read-modify-write is a single atomic PostgreSQL statement,
     * eliminating the race condition that exists when doing a separate
     * findLamportClock() + saveLamportClock() across two HTTP calls.
     *
     * @param nodeId    the EPS node identifier
     * @param candidate the device timestamp in epoch ms to compare against
     * @return the new Lamport sequence number assigned to this event
     */
    @Override
    public long incrementAndGetLamportClock(String nodeId, long candidate) {
        String sql = """
            INSERT INTO eps_lamport_clock (node_id, clock_value)
            VALUES (?, GREATEST(?, 0) + 1)
            ON CONFLICT (node_id) DO UPDATE
                SET clock_value = GREATEST(
                    eps_lamport_clock.clock_value,
                    ?,
                    EXCLUDED.clock_value - 1
                ) + 1
            RETURNING clock_value
            """;
        try (Connection conn = dataSource.getConnection();
             PreparedStatement ps = conn.prepareStatement(sql)) {
            ps.setString(1, nodeId);
            ps.setLong(2, candidate);
            ps.setLong(3, candidate);
            ResultSet rs = ps.executeQuery();
            if (rs.next()) return rs.getLong("clock_value");
            throw new RuntimeException("RETURNING clause returned no rows");
        } catch (SQLException e) {
            throw new RuntimeException("Failed to atomically increment Lamport clock", e);
        }
    }
    ...
    public void close() {
        if (dataSource != null && !dataSource.isClosed()) {
            dataSource.close();
            System.out.println("[StorageServiceImpl] Connection pool closed");
        }
    }
